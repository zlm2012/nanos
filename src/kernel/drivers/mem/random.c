...
static uint32_t random_pool[POOLWORDS];
static struct timer_rand_state keyboard_timer_state;
static struct timer_rand_state extract_timer_state;
static void rand_driver_thread(void);
				 ...
				 struct timer_rand_state *state, unsigned num)
{
  ...
  add_entropy_word(r, (uint32_t) num);
				  ...
				  int nbytes)
{
  ...
  while (nbytes) {
    ...
    i = MIN(nbytes, 16);
    memcpy(buf, (uint8_t const *)tmp, i);
    nbytes -= i;
    buf += i;
  }
...
rand_driver_thread(void) {
  ...
  while (true) {
    ...
    if (m.type == DEV_READ) {
      ...
      uint8_t data;
      if (m.len%4) {
	copy_to_kernel(fetch_pcb(m.req_pid), &data, m.buf, 4);
	add_entropy_word(&random_state, (uint32_t)data);
	m.buf+=m.len%4;
	m.len/=4;
      }
      for (i = 0; i < m.len; i ++) {
	copy_to_kernel(fetch_pcb(m.req_pid), &data, m.buf + i*4, 4);
	add_entropy_word(&random_state, (uint32_t)data);
      }
      m.ret = i;
      m.dest = m.src;
